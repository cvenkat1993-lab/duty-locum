...
export default function BrowseJobsPage() {
  ...
  const loadJobs = async () => {
    try {
      ...
      // Extract unique values for filters - filter out undefined/null values
      const titles = Array.from(new Set(jobs.map(j => j.title).filter((t): t is string => Boolean(t))));
      const departments = Array.from(new Set(jobs.map(j => j.department).filter((d): d is string => Boolean(d))));
      const workTypes = Array.from(new Set(jobs.map(j => j.workType).filter(Boolean)));

      setUniqueJobTitles(titles.sort());
      setUniqueDepartments(departments.sort());
      setUniqueWorkTypes(workTypes.sort());
    } catch (error) {
